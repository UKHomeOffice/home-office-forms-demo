module.exports = {
  getstaticSfiaSkills: function () {
    return [
      // *******************ARCHITECTURE*****************
      // Business Architecture
      {
        value: 'Design',
        label: 'Design'
      },
      {
        value: 'Architecture Breadth',
        label: 'Architecture Breadth'
      },
      {
        value: 'Architecture',
        label: 'Architecture'
      },
      {
        value: 'Depth',
        label: 'Depth'
      },
      {
        value: 'Innovation',
        label: 'Innovation'
      },
      {
        value: 'Delivery',
        label: 'Delivery'
      },
      {
        value: 'Method and Standards Driven Approach',
        label: 'Method and Standards Driven Approach'
      },


      // Data Architecture
      {
        value: 'INOV - Innovation',
        label: 'INOV - Innovation'
      },
      {
        value: 'ITSP - Strategic planning',
        label: 'ITSP - Strategic planning'
      },
      {
        value: 'IRMG - Information Governance',
        label: 'IRMG - Information Governance'
      },
      {
        value: 'DTAN - Data Modelling and Design',
        label: 'DTAN - Data Modelling and Design'
      },
      {
        value: 'CNSL - Consultancy',
        label: 'CNSL - Consultancy'
      },
      {
        value: 'STPL - Enterprise and Business Architecture',
        label: 'STPL - Enterprise and Business Architecture'
      },


      // Security Architecture
      {
        value: 'ITSP - Strategic planning',
        label: 'ITSP - Strategic planning'
      },
      {
        value: 'CNSL - Consultancy',
        label: 'CNSL - Consultancy'
      },
      {
        value: 'TECH - Specialist Advise',
        label: 'TECH - Specialist Advise'
      },
      {
        value: 'INOV - Innovation',
        label: 'INOV - Innovation'
      },
      {
        value: 'STPL - Enterprise and Business Architecture',
        label: 'STPL - Enterprise and Business Architecture'
      },
      {
        value: 'ARCH - Solution Architecture',
        label: 'ARCH - Solution Architecture'
      },


      // Technical Architecture
      {
        value: 'ITSP - Strategic planning',
        label: 'ITSP - Strategic planning'
      },
      {
        value: 'CNSL - Consultancy',
        label: 'CNSL - Consultancy'
      },
      {
        value: 'TECH - Specialist Advise',
        label: 'TECH - Specialist Advise'
      },
      {
        value: 'INOV - Innovation',
        label: 'INOV - Innovation'
      },
      {
        value: 'STPL - Enterprise and Business Architecture',
        label: 'STPL - Enterprise and Business Architecture'
      },
      {
        value: 'ARCH - Solution Architecture',
        label: 'ARCH - Solution Architecture'
      },

      // *******************DATA*****************
      //	Data Analysis
      {
        value: 'INAN - Analytics',
        label: 'INAN - Analytics'
      },
      {
        value: 'DTAN - Data Modelling and Design',
        label: 'DTAN - Data Modelling and Design'
      },
      {
        value: 'PROG - Programming/Software Development',
        label: 'PROG - Programming/Software Development'
      },
      {
        value: 'INOV - Innovation',
        label: 'INOV - Innovation'
      },
      {
        value: 'QUAS - Quality Assurance',
        label: 'QUAS - Quality Assurance'
      },
      {
        value: 'RLMT - Relationship Management',
        label: 'RLMT - Relationship Management'
      },


      // Data Engineering
      {
        value: 'INOV - Innovation',
        label: 'INOV - Innovation'
      },
      {
        value: 'DTAN - Data Modelling and Design',
        label: 'DTAN - Data Modelling and Design'
      },
      {
        value: 'DBDS - Database design',
        label: 'DBDS - Database design'
      },
      {
        value: 'METL - Methods and Tools',
        label: 'METL - Methods and Tools'
      },
      {
        value: 'DATM - Data Management',
        label: 'DATM - Data Management'
      },
      {
        value: 'DBAD - Database Administration',
        label: 'DBAD - Database Administration'
      },


      // Data Science
      {
        value: 'INOV - Innovation',
        label: 'INOV - Innovation'
      },
      {
        value: 'INAN - Analytics',
        label: 'INAN - Analytics'
      },
      {
        value: 'DTAN - Data Modelling and Design',
        label: 'DTAN - Data Modelling and Design'
      },
      {
        value: 'PROG - Programming/Software Development',
        label: 'PROG - Programming/Software Development'
      },
      {
        value: 'Professional standards & quality',
        label: 'Professional standards & quality'
      },
      {
        value: 'Achieve Impact with Analysis',
        label: 'Achieve Impact with Analysis'
      },

      // Head of Role for Data
      {
        value: 'INAN - Analytics',
        label: 'INAN - Analytics'
      },
      {
        value: 'INOV - Innovation',
        label: 'INOV - Innovation'
      },
      {
        value: 'DATM - Data Management',
        label: 'DATM - Data Management'
      },
      {
        value: 'DTAN - Data Modelling and Design',
        label: 'DTAN - Data Modelling and Design'
      },
      {
        value: 'QUAS - Quality Assurance',
        label: 'QUAS - Quality Assurance'
      },
      {
        value: 'RLMT - Relationship Management',
        label: 'RLMT - Relationship Management'
      },

      // *******************DELIVERY*****************
      // Technology Delivery Manager, Outcome Technology Delivery Manager AND Head of Role for Delivery
      {
        value: 'Project Management',
        label: 'Project Management'
      },
      {
        value: 'Quality Management',
        label: 'Quality Management'
      },
      {
        value: 'Business Risk Management',
        label: 'Business Risk Management'
      },
      {
        value: 'Relationship Management',
        label: 'Relationship Management'
      },
      {
        value: 'Financial and Commercial Management',
        label: 'Financial and Commercial Management'
      },
      {
        value: 'Continuous Improvement',
        label: 'Continuous Improvement'
      },
      {
        value: 'Governance',
        label: 'Governance'
      },
      // *******************ENGINEERING*****************
      // Infrastructure Engineering
      {
      	value: 'ITMG - IT management',
      	label: 'ITMG - IT management'
      },
      {
        value: 'DESN - Systems design',
        label: 'DESN - Systems design'
      },
      {
      	value: 'HSIN - Systems installation/decommissioning',
      	label: 'HSIN - Systems installation/decommissioning'
      },
      {
        value: 'ITOP - IT infrastructure',
        label: 'ITOP - IT infrastructure'
      },
      {
        value: 'PBMG - Problem management',
        label: 'PBMG - Problem management'
      },
      {
      	value: 'USUP - Incident management',
      	label: 'USUP - Incident management'
      },

      // Network Engineering
      {
        value: 'TECH - Specialist Advise',
        label: 'TECH - Specialist Advise'
      },
      {
        value: 'ITMG - IT management',
        label: 'ITMG - IT management'
      },
      {
        value: 'NTDS - Network Design',
        label: 'NTDS - Network Design'
      },
      {
        value: 'HSIN - Systems installation/decommissioning',
        label: 'HSIN - Systems installation/decommissioning'
      },
      {
        value: 'PBMG - Problem management',
        label: 'PBMG - Problem management'
      },
      {
      	value: 'USUP - Incident management',
      	label: 'USUP - Incident management'
      },

      // Software Engineering and Devops
      {
        value: 'DATM - Data Management',
        label: 'DATM - Data Management'
      },
      {
        value: 'ASUP - Application Support',
        label: 'ASUP - Application Support'
      },
      {
        value: 'TEST - Testing',
        label: 'TEST - Testing'
      },
      {
        value: 'PROG - Programming/Software Development',
        label: 'PROG - Programming/Software Development'
      },
      {
        value: 'SWDN - Software Design',
        label: 'SWDN - Software Design'
      },
      {
        value: 'SINT - Systems Integration and Build',
        label: 'SINT - Systems Integration and Build'
      },
      {
        value: 'DLMG - Systems Development Management',
        label: 'DLMG - Systems Development Management'
      },
      {
        value: 'PEMT - Performance Management',
        label: 'PEMT - Performance Management'
      },
      {
        value: 'PDSV - Professional Development',
        label: 'PDSV - Professional Development'
      },
      {
        value: 'QUMG - Quality Management',
        label: 'QUMG - Quality Management'
      },
      {
        value: 'METL - Methods and Tools',
        label: 'METL - Methods and Tools'
      },
      {
        value: 'ETMG - Learning and Development Management',
        label: 'ETMG - Learning and Development Management'
      },
      {
        value: 'OCDV - Organisational Capability Development',
        label: 'OCDV - Organisational Capability Development'
      },

      // Security Engineering
      {
        value: 'SCAD - Security Administration',
        label: 'SCAD - Security Administration'
      },
      {
      	value: 'USUP - Incident management',
      	label: 'USUP - Incident management'
      },
      {
      	value: 'SCTY - Information Security',
      	label: 'SCTY - Information Security'
      },
      {
      	value: 'INAN - Analytics',
      	label: 'INAN - Analytics'
      },
      {
        value: 'PENT - Penetration Testing',
        label: 'PENT - Penetration Testing'
      },
      {
        value: 'DGFS - Digital Forensics',
        label: 'DGFS - Digital Forensics'
      },
      // Engineering Leadership
      {
        value: 'DLMG - Systems Development Management',
        label: 'DLMG - Systems Development Management'
      },
      {
        value: 'SWDN - Software Design',
        label: 'SWDN - Software Design'
      },
      {
        value: 'PROG - Programming/Software Development',
        label: 'PROG - Programming/Software Development'
      },
      {
        value: 'PEMT - Performance Management',
        label: 'PEMT - Performance Management'
      },
      {
        value: 'PDSV - Professional Development',
        label: 'PDSV - Professional Development'
      },
      {
        value: 'QUMG - Quality Management',
        label: 'QUMG - Quality Management'
      },


      // Head of Role for Engineering
      {
        value: 'METL - Methods and Tools',
        label: 'METL - Methods and Tools'
      },
      {
        value: 'SWDN - Software Design',
        label: 'SWDN - Software Design'
      },
      {
        value: 'PROG - Programming/Software Development',
        label: 'PROG - Programming/Software Development'
      },
      {
        value: 'PDSV - Professional Development',
        label: 'PDSV - Professional Development'
      },
      {
        value: 'ETMG - Emerging Technology Monitoring',
        label: 'ETMG - Emerging Technology Monitoring'
      },
      {
        value: 'OCDV - Organisational Capability Development',
        label: 'OCDV - Organisational Capability Development'
      },

      // *******************IT OPERATIONS*****************
      // Plan, Engage & Improve
      {
        value: 'TECH - Specialist Advise',
        label: 'TECH - Specialist Advise'
      },
      {
        value: 'SLMO - Service Level Management',
        label: 'SLMO - Service Level Management'
      },
      {
        value: 'RLMT - Relationship Management',
        label: 'RLMT - Relationship Management'
      },
      {
        value: 'BURM - Business Risk Management',
        label: 'BURM - Business Risk Management'
      },
      {
        value: 'MEAS - Measurement',
        label: 'MEAS - Measurement'
      },
      {
        value: 'FMIT - Financial Management',
        label: 'FMIT - Financial Management'
      },
      {
        value: 'SUPP - Supplier Management',
        label: 'SUPP - Supplier Management'
      },
      {
        value: 'CSMG - Customer Service Support',
        label: 'CSMG - Customer Service Support'
      },
      {
        value: 'COPL - Continuity Management',
        label: 'COPL - Continuity Management'
      },
      {
        value: 'SCTY - Information Security',
        label: 'SCTY - Information Security'
      },
      {
        value: 'CPMG - Capacity Management',
        label: 'CPMG - Capacity Management'
      },
      {
        value: 'AVMT - Availability Management',
        label: 'AVMT - Availability Management'
      },
      {
        value: 'DEMM - Demand Management',
        label: 'DEMM - Demand Management'
      },

      //	Design & Transition
      {
        value: 'ITMG - IT Management',
        label: 'ITMG - IT Management'
      },
      {
        value: 'BUAN - Business Analysis',
        label: 'BUAN - Business Analysis'
      },
      {
        value: 'SEAC - Service Acceptance',
        label: 'SEAC - Service Acceptance'
      },
      {
        value: 'RLMT - Relationship Management',
        label: 'RLMT - Relationship Management'
      },
      {
        value: 'METL - Methods and Tools',
        label: 'METL - Methods and Tools'
      },
      {
        value: 'PRMG - Project Management',
        label: 'PRMG - Project Management'
      },
      {
        value: 'CFMG - Configuration Management',
        label: 'CFMG - Configuration Management'
      },
      {
        value: 'ASMG - Asset Management',
        label: 'ASMG - Asset Management'
      },
      {
        value: 'CHMG - Change Management',
        label: 'CHMG - Change Management'
      },
      {
        value: 'RELM - Release Management',
        label: 'RELM - Release Management'
      },

      // Deliver & Support
      {
        value: 'KNOW - Knowledge Management',
        label: 'KNOW - Knowledge Management'
      },
      {
        value: 'SLMO - Service Level Management',
        label: 'SLMO - Service Level Management'
      },
      {
        value: 'ITOP - IT infrastructure',
        label: 'ITOP - IT infrastructure'
      },
      {
        value: 'PBMG - Problem management',
        label: 'PBMG - Problem management'
      },
      {
        value: 'USUP - Incident management',
        label: 'USUP - Incident management'
      },
      {
        value: 'CSMG - Customer Service Support',
        label: 'CSMG - Customer Service Support'
      },

      // IT Operations Leadership
      {
        value: 'TECH - Specialist Advise',
        label: 'TECH - Specialist Advise'
      },
      {
        value: 'SLMO - Service Level Management',
        label: 'SLMO - Service Level Management'
      },
      {
        value: 'RLMT - Relationship Management',
        label: 'RLMT - Relationship Management'
      },
      {
        value: 'FMIT - Financial Management',
        label: 'FMIT - Financial Management'
      },
      {
        value: 'ITSP - Strategic planning',
        label: 'ITSP - Strategic planning'
      },
      {
        value: 'PDSV - Professional Development',
        label: 'PDSV - Professional Development'
      },
      {
        value: 'PEMT - Performance Management',
        label: 'PEMT - Performance Management'
      },
      {
        value: 'ETMG - Learning and Development Management',
        label: 'ETMG - Learning and Development Management'
      },
      {
        value: 'RESC - Resourcing',
        label: 'RESC - Resourcing'
      },

      // *******************PRODUCT*****************
      // Business Analyst
      {
        value: 'BUAN - Business analysis',
        label: 'BUAN - Business analysis'
      },
      {
        value: 'BSMO - Business modelling',
        label: 'BSMO - Business modelling'
      },
      {
        value: 'INOV - Innovation',
        label: 'INOV - Innovation'
      },
      {
        value: 'RLMT - Relationship Management',
        label: 'RLMT - Relationship Management'
      },
      {
        value: 'REQM - Requirements definition and management',
        label: 'REQM - Requirements definition and management'
      },
      {
        value: 'USEV - User experience evaluation',
        label: 'USEV - User experience evaluation'
      },


      // Product Manager
      {
        value: 'BPRE - Business process improvement',
        label: 'BPRE - Business process improvement'
      },
      {
        value: 'INOV - Innovation',
        label: 'INOV - Innovation'
      },
      {
        value: 'PROD - Product management',
        label: 'PROD - Product management'
      },
      {
        value: 'REQM - Requirements definition and management',
        label: 'REQM - Requirements definition and management'
      },
      {
        value: 'DLMG - Systems development management',
        label: 'DLMG - Systems development management'
      },
      {
        value: 'UNAN - User experience analysis',
        label: 'UNAN - User experience analysis'
      },


      // Performance Analysis
      {
        value: 'TECH - Specialist Advice',
        label: 'TECH - Specialist Advice'
      },
      {
        value: 'INAN - Analytics',
        label: 'INAN - Analytics'
      },
      {
        value: 'RLMT - Relationship Management',
        label: 'RLMT - Relationship Management'
      },
      {
        value: 'VISL - Data Visualisation',
        label: 'VISL - Data Visualisation'
      },
      {
        value: 'METL - Methods and Tools',
        label: 'METL - Methods and Tools'
      },
      {
        value: 'DATM - Data Management',
        label: 'DATM - Data Management'
      },

      // *******************QUALITY ASSURANCE AND TESTING*****************
      // QAT
      {
        value: 'ITSP - Strategic Planning',
        label: 'ITSP - Strategic Planning'
      },
      {
        value: 'Test Analysis, Design and Execution',
        label: 'Test Analysis, Design and Execution'
      },
      {
        value: 'Automation and Tooling',
        label: 'Automation and Tooling'
      },
      {
        value: 'Performance Testing',
        label: 'Performance Testing'
      },
      {
        value: 'Integration and Assurance',
        label: 'Integration and Assurance'
      },
      {
        value: 'Domain Experience',
        label: 'Domain Experience'
      },
      // *******************USER CENTRED DESIGN*****************
      // Content Designer
      {
        value: 'Content Design',
        label: 'Content Design'
      },
      {
        value: 'Negotiating and Influencing',
        label: 'Negotiating and Influencing'
      },
      {
        value: 'Content Planning and Strategy',
        label: 'Content Planning and Strategy'
      },
      {
        value: 'User Focus',
        label: 'User Focus'
      },
      {
        value: 'Inclusion',
        label: 'Inclusion'
      },
      {
        value: 'Technical Breadth',
        label: 'Technical Breadth'
      },
      // Interaction Designer
      {
        value: 'Interaction Design',
        label: 'Interaction Design'
      },
      {
        value: 'User Centred Design',
        label: 'User Centred Design'
      },
      {
        value: 'Prototyping',
        label: 'Prototyping'
      },
      {
        value: 'Interfaces',
        label: 'Interfaces'
      },
      {
        value: 'User Focus',
        label: 'User Focus'
      },
      {
        value: 'Inclusion',
        label: 'Inclusion'
      },
      {
        value: 'Technical Breadth',
        label: 'Technical Breadth'
      },

      // Service
      {
        value: 'Service Analysis',
        label: 'Service Analysis'
      },
      {
        value: 'User Centred Design',
        label: 'User Centred Design'
      },
      {
        value: 'Measurement',
        label: 'Measurement'
      },
      {
        value: 'Prototyping',
        label: 'Prototyping'
      },
      {
        value: 'Assisted Digital and Inclusion',
        label: 'Assisted Digital and Inclusion'
      },
      {
        value: 'Technical Breadth',
        label: 'Technical Breadth'
      },

      // User Researcher
      {
        value: 'Research Methods',
        label: 'Research Methods'
      },
      {
        value: 'Analysing and Sharing Research',
        label: 'Analysing and Sharing Research'
      },
      {
        value: 'Ethics',
        label: 'Ethics'
      },
      {
        value: 'Inclusion',
        label: 'Inclusion'
      },
      {
        value: 'Negotiating and Influencing',
        label: 'Negotiating and Influencing'
      },
      {
        value: 'Technical Breadth',
        label: 'Technical Breadth'
      },

      // Accessibility & Digital Inclusion
      {
        value: 'User Focus',
        label: 'User Focus'
      },
      {
        value: 'Communicating Information',
        label: 'Communicating Information'
      },
      {
        value: 'Leadership & Guidance',
        label: 'Leadership & Guidance'
      },
      {
        value: 'Test Analysis, Design and Execution',
        label: 'Test Analysis, Design and Execution'
      },
      {
        value: 'Accessibility Policy, Standards and Assurance',
        label: 'Accessibility Policy, Standards and Assurance'
      },
      {
        value: 'Digital Inclusion',
        label: 'Digital Inclusion'
      }
    ];
  },

  // Individual skills
  getstaticAccessibilityandDigitalInclusionSfiaSkills: function () {
    return [
      'User Focus',
      'Communicating Information',
      'Leadership & Guidance',
      'Test Analysis, Design and Execution',
      'Accessibility Policy, Standards and Assurance',
      'Digital Inclusion'
    ];
  },
  getstaticBusinessAnalystSfiaSkills: function () {
    return [
      'BUAN - Business analysis',
      'BSMO - Business modelling',
      'INOV - Innovation',
      'RLMT - Relationship Management',
      'REQM - Requirements definition and management',
      'USEV - User experience evaluation'
    ];
  },
  getBusinessArchitectureSkills: function () {
    return [
      'Design',
      'Architecture Breadth',
      'Architecture',
      'Depth',
      'Innovation',
      'Delivery',
      'Method and Standards Driven Approach'
    ];
  },
  getstaticContentDesignerSfiaSkills: function () {
    return [
      'Content Design',
      'Negotiating and Influencing',
      'Content Planning and Strategy',
      'User Focus',
      'Inclusion',
      'Technical Breadth'
    ];
  },
  getstaticDataAnalystSfiaSkills: function () {
    return [
      'INAN - Analytics',
      'DTAN - Data Modelling and Design',
      'PROG - Programming/software development',
      'INOV - Innovation',
      'QUAS - Quality Assurance',
      'RLMT - Relationship Management'
    ];
  },
  getDataArchitectureSkills: function () {
    return [
      'INOV - Innovation',
      'ITSP - Strategic planning',
      'IRMG - Information Governance',
      'DTAN - Data Modelling and Design',
      'CNSL - Consultancy',
      'STPL - Enterprise and Business Architecture'
    ];
  },
  getstaticDataEngineeringSfiaSkills: function () {
    return [
      'INOV - Innovation',
      'DTAN - Data Modelling and Design',
      'DBDS - Database design',
      'METL - Methods and Tools',
      'DATM - Data Management',
      'DBAD - Database Administration'
    ];
  },
  getstaticDataScienceSfiaSkills: function () {
    return [
      'INOV - Innovation',
      'INAN - Analytics',
      'DTAN - Data Modelling and Design',
      'PROG - Programming/Software Development',
      'Professional standards & quality',
      'Achieve Impact with Analysis'
    ];
  },
  getstaticDeliverAndSupportSfiaSkills: function () {
    return [
      'KNOW - Knowledge Management',
      'SLMO - Service Level Management',
      'ITOP - IT infrastructure',
      'PBMG - Problem management',
      'USUP - Incident management',
      'CSMG - Customer Service Support'
    ];
  },
  getStaticDeliverySfiaSkills: function () {
    return [
      'Project Management',
      'Quality Management',
      'Business Risk Management',
      'Relationship Management',
      'Financial and Commercial Management',
      'Continuous Improvement',
      'Governance'
    ];
  },
  getstaticDesignAndTransitionSfiaSkills: function () {
    return [
      'ITMG - IT Management',
      'BUAN - Business Analysis',
      'SEAC - Service Acceptance',
      'RLMT - Relationship Management',
      'METL - Methods and Tools',
      'PRMG - Project Management',
      'CFMG - Configuration Management',
      'ASMG - Asset Management',
      'CHMG - Change Management',
      'RELM - Release Management'
    ];
  },
  getstaticEngineeringLeadershipSfiaSkills: function () {
    return [
      'DLMG - Systems Development Management',
      'PROG - Programming/Software Development',
      'SWDN - Software Design',
      'PEMT - Performance Management',
      'PDSV - Professional Development',
      'QUMG - Quality Management'
    ];
  },
  getstaticHeadOfRoleForDataSfiaSkills: function () {
    return [
      'INAN - Analytics',
      'INOV - Innovation',
      'DATM - Data Management',
      'DTAN - Data Modelling and Design',
      'QUAS - Quality Assurance',
      'RLMT - Relationship Management'
    ];
  },
  getstaticHeadOfRoleForEngineeringSfiaSkills: function () {
    return [
      'PROG - Programming/Software Development',
      'SWDN - Software Design',
      'PDSV - Professional Development',
      'METL - Methods and Tools',
      'ETMG - Learning and Development Management',
      'OCDV - Organisational Capability Development'
    ];
  },
  getstaticInteractionDesignerSfiaSkills: function () {
    return [
      'Interaction Design',
      'User Centred Design',
      'Prototyping',
      'Interfaces',
      'User Focus',
      'Inclusion',
      'Technical Breadth'
    ];
  },
  getstaticInfrastructureEngineeringSfiaSkills: function () {
    return [
      'ITMG - IT management',
      'DESN - Systems design',
      'HSIN - Systems installation/decommissioning',
      'ITOP - IT infrastructure',
      'PBMG - Problem management',
      'USUP - Incident management'
    ];
  },
  getstaticItOperationsLeadershipSfiaSkills: function () {
    return [
      'TECH - Specialist Advise',
      'SLMO - Service Level Management',
      'RLMT - Relationship Management',
      'FMIT - Financial Management',
      'ITSP - Strategic planning',
      'PDSV - Professional Development',
      'PEMT - Performance Management',
      'ETMG - Learning and Development Management',
      'RESC - Resourcing'
    ];
  },
  getstaticNetworkingEngineeringSfiaSkills: function () {
    return [
      'TECH - Specialist Advise',
      'ITMG - IT management',
      'NTDS - Network Design',
      'HSIN - Systems installation/decommissioning',
      'PBMG - Problem management',
      'USUP - Incident management'
    ];
  },
  getstaticPerformanceAnalysisSfiaSkills: function () {
    return [
      'TECH - Specialist Advice',
      'INAN - Analytics',
      'RLMT - Relationship Management',
      'VISL - Data Visualisation',
      'METL - Methods and Tools',
      'DATM - Data Management'
    ];
  },
  getstaticPlanEngageAndImproveSfiaSkills: function () {
    return [
      'TECH - Specialist Advise',
      'SLMO - Service Level Management',
      'RLMT - Relationship Management',
      'BURM - Business Risk Management',
      'MEAS - Measurement',
      'FMIT - Financial Management',
      'SUPP - Supplier Management',
      'CSMG - Customer Service Support',
      'COPL - Continuity Management',
      'SCTY - Information Security',
      'CPMG - Capacity Management',
      'AVMT - Availability Management',
      'DEMM - Demand Management'
    ];
  },
  getstaticProductManagerSfiaSkills: function () {
    return [
      'BPRE - Business process improvement',
      'INOV - Innovation',
      'PROD - Product management',
      'REQM - Requirements definition and management',
      'DLMG - Systems development management',
      'UNAN - User experience analysis'
    ];
  },
  getstaticQualityAssuranceAndTestingSfiaSkills: function () {
    return [
      'ITSP - Strategic planning',
      'Test Analysis, Design and Execution',
      'Automation and Tooling',
      'Performance Testing',
      'Integration and Assurance',
      'Domain Experience'
    ];
  },
  getSecurityArchitectureSkills: function () {
    return [
      'ITSP - Strategic planning',
      'CNSL - Consultancy',
      'TECH - Specialist Advise',
      'INOV - Innovation',
      'STPL - Enterprise and Business Architecture',
      'ARCH - Solution Architecture'
    ];
  },
  getstaticSecurityEngineeringSfiaSkills: function () {
    return [
      'SCAD - Security Administration',
      'USUP - Incident management',
      'SCTY - Information Security',
      'INAN - Analytics',
      'PENT - Penetration Testing',
      'DGFS - Digital Forensics'
    ];
  },
  getstaticServiceSfiaSkills: function () {
    return [
      'Service Analysis',
      'User Centred Design',
      'Measurement',
      'Prototyping',
      'Assisted Digital and Inclusion',
      'Technical Breadth'
    ];
  },
  getstaticSoftwareAndDevOpsSfiaSkills: function () {
    return [
      'DATM - Data Management',
      'ASUP - Application Support',
      'TEST - Testing',
      'PROG - Software Development',
      'SWDN - Software Design',
      'SINT - Systems Integration and Build',
      'DLMG - Systems Development Management',
      'PEMT - Performance Management',
      'PDSV - Professional Development',
      'QUMG - Quality Management',
      'METL - Methods and Tools',
      'ETMG - Learning and Development Management',
      'OCDV - Organisational Capability Development'
    ];
  },
  getTechnicalArchitectureSkills: function () {
    return [
      'ITSP - Strategic planning',
      'CNSL - Consultancy',
      'TECH - Specialist Advise',
      'INOV - Innovation',
      'STPL - Enterprise and Business Architecture',
      'ARCH - Solution Architecture'
    ];
  },
  getstaticUserResearcherSfiaSkills: function () {
    return [
      'Research Methods',
      'Analysing and Sharing Research',
      'Ethics',
      'Inclusion',
      'Negotiating and Influencing',
      'Technical Breadth'
    ];
  }
};
